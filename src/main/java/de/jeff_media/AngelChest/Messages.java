...
public class Messages {
	...
	Messages(Main main) {
		...
		HOLOGRAM_TEXT = ChatColor.translateAlternateColorCodes('&', main.getConfig().getString("hologram-text","&a&l[AngelChest]&r\n&b{player}\n&6{time}"));
		
		ANGELCHEST_INVENTORY_NAME = ChatColor.translateAlternateColorCodes('&', main.getConfig().getString("angelchest-inventory-name","&a[AngelChest] &b{player}&r"));

		ANGELCHEST_LIST = ChatColor.translateAlternateColorCodes('&', main.getConfig().getString("angelchest-list","[{id}] {time} &aX:&f {x} &aY:&f {y} &aZ:&f {z} | {world} "));
